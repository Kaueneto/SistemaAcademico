    ...
    public class TurmaController {
        ...
        private boolean validarEntrada() {
            String semestre  = txtSemestre.getText();
            String horario   = txtHorario.getText();
            Disciplina disc  = cboxDisciplina.getValue();
            Professor  prof  = cboxProf.getValue();

            if (semestre.isBlank() || horario.isBlank() || disc == null || prof == null) {
                alert("Preencha todos os campos.", Alert.AlertType.WARNING);
                return false;
            }
            if (!horaValida(horario)) {
                alert("Horário inválido. Use o formato HH:MM (exemplo: 19:00).", Alert.AlertType.WARNING);
